...
#-----------------                
                    ...
                    for cl in np.unique(t_out['cluster_label']): #for every unique validated cluster,
                        ...
                        if pluster_length>cluster_length: #if there are any new predicted track members,
                            ...
                            n=n        
            #t_out.write("hgroup_%d/healpix_%d.fits" % (subdir,int(pix)),format="fits",overwrite=True)
            t_out.write("../../../../mnt/lustrefs/scratch/katie.fasbender/nsc_fast_track_healpix/hgroup_%d/healpix_%d.fits" % (subdir,int(pix)),format="fits",overwrite=True) #write a fits file with all measurements not associated with SOs, and their info
        else:
            tr_out=dat[outliers]
            tr_out.add_column(Column(np.repeat(-1,len(tr_out))),name='cluster_label')
            ...
            #tr_out.write("hgroup_%d/healpix_%d.fits" % (subdir,int(pix)),format="fits",overwrite=True)
            tr_out.write("../../../../mnt/lustrefs/scratch/katie.fasbender/nsc_fast_track_healpix/hgroup_%d/healpix_%d.fits" % (subdir,int(pix)),format="fits",overwrite=True)
    #else:
   # 	print("nope!")

...
#-------------------
        ...
        X_out.add_column(Column(np.zeros(len(X_out),dtype=float)),name="v_ra") #add RA velocity column to out_table
        X_out.add_column(Column(np.zeros(len(X_out),dtype=float)),name="v_dec") #add Dec velocity column to out_table
        X_out.add_column(Column(np.repeat(pix,len(X_out))),name='pix') #give them a healpix label
        X_out.add_column(Column(np.repeat(-1,len(X_out))),name='track_p') #give it a track_p numbering column
        X_out.add_column(Column(np.repeat(-1,len(X_out))),name='track_h') #give it a track_h numbering column
        if mark=1: #if there's adequate exposure spacing:
#--------------------
# Tracklet Formation 
#--------------------
...
#-----------------                
                    ...
                    for cl in np.unique(t_out['cluster_label']): #for every unique validated cluster,
                        ...
                        if pluster_length>cluster_length: #if there are any new predicted track members,
                            ...
                            n=n        
            t_out.write("../../../../mnt/lustrefs/scratch/katie.fasbender/nsc_fast_track_healpix/healpix_%d.fits" % int(pix),format="fits",overwrite=True) #write a fits file with all measurements not associated with SOs, and their info
        else:
            X_out.add_column(Column(np.repeat(-1,len(X_out))),name='cluster_label')
            X_out.write("../../../../mnt/lustrefs/scratch/katie.fasbender/nsc_fast_track_healpix/healpix_%d.fits" % int(pix),format="fits",overwrite=True)

...
## All other lines are read by the shell.
	...
	with open(job_file,'w') as fh: #even if the file doesn't exist, it will be created and the lines will be written to it!!!!!! <3
		...
		fh.writelines("#SBATCH --output=healpix_%d-%j.out\n" % my_set) # standard output file (%j = jobid)
		fh.writelines("#SBATCH --error=healpix_%d-%j.err\n" % my_set) # standard error file
		...
		for i in range(0,max_data): #loop through elements of input file
			if n<2:
				fh.writelines("python nsc_sso_find.py %d %d" % (hdul[1].data[i][0],hdul[1].data[i][1])) #this is the command that runs the python script to find SSOs for 1 healpix
				n=n+1
